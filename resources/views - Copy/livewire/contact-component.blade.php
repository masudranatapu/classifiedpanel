                    ...
                    <form wire:submit.prevent="submitContact" method="Post">
                        ...
                        <div class="input-field--textarea">
                            <textarea placeholder="{{ __('message') }}" name="message" wire:model.defer="message"
                                class="form-control @error('message') is-invalid @enderror" style="resize: none"></textarea>
                            @error('message')
                                <p class="text-danger text-sm"> {{ $message }} </p>
                            @enderror
                        </div>
                        @if (config('captcha.active'))
                            <div wire:model.defer="g-recaptcha-response" name="g-recaptcha-response"
                                class="input-field--textarea">
                                {!! NoCaptcha::display() !!}
                                <span id="recaptcha-error" class="d-none text-danger text-sm">
                                    <strong>Please verify that you are not a robot.</strong>
                                </span>
                            </div>
                        @endif
                        <button class="btn submit-btn" type="submit">{{ __('send_message') }}</button>
                    </form>
                </div>
            </div>
        </div>
