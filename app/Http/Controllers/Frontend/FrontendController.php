...
class FrontendController extends Controller
{
    ...
    public function homePage1($data)
    {
        ...
        $recommendedAds = AdResource::collection($ad_data->where('featured', 1)->take(12)->latest()->get());
        $latestAds = AdResource::collection($ad_data->take(12)->latest()->get());
        ...
        // return [
        //     'recommendedAds' => $data['recommendedAds'],
    ...
    public function about()
    {
        $testimonials = Testimonial::latest('id')
            ->where('code', currentLangCode())->get();
    ...
    public function faq()
    {
        ...
        $first_faq = FaqCategory::whereHas('faqs', function ($q) {
            $q->where('code', currentLangCode());
        })->first();

        $category_slug = request('category') ?? ($first_faq ? $first_faq->slug : '');
        ...
        $data['faqs'] = Faq::where('faq_category_id', $category ? $category->id : '')
            ->where('code', currentLangCode())
            ->with('faq_category:id,name,icon')->get();
    ...
    public function adDetails(Ad $ad)
    {
        ...
        if ($ad->status === 'sold' && $ad->customer->id !== auth('user')->id()) {
            ...
            return view('frontend.single-ad', compact('ad', 'lists', 'product_custom_field_groups'));
        }
    }
